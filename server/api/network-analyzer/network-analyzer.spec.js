...
describe('Network-Analyzer API', function() {
  ...
  describe('\n\t GET /api/network-analyzers', function() {
  ...
  describe('\n\t GET /api/network-analyzers/:id', function () {
    it('should respond with a unauthorized error when do not have a token', function (done) {
      request(server)
        .get('/api/network-analyzers/' + analyzer._id)
        .expect(401)
        .expect('Content-Type', /html/)
        .end(function (err, res) {
          if (err) { return done(err); }
    ...
    it('should respond with bad request error when have a incorrect id', function (done) {
      request(server)
        .get('/api/network-analyzers/' + 'wrong analyzerId')
        .set(base)
        .expect(500)
        .expect('Content-Type', /json/)
        .end(function (err, res) {
  ...
  describe('\n\t PUT /api/network-analyzers/:id', function () {
    it('should respond with a unauthorized error when do not have a token', function (done) {
      request(server)
        .put('/api/network-analyzers/' + analyzer._id)
        .send(analyzer)
        .expect(401)
        .expect('Content-Type', /html/)
        .end(function (err, res) {
    ...
    it('should respond with bad request error when have a incorrect id', function (done) {
      request(server)
        .put('/api/network-analyzers/' + 'wrong testId')
        .set(base)
        .send(analyzer)
        .expect(500)
        .expect('Content-Type', /json/)
